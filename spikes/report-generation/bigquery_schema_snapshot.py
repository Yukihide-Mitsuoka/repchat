...
def _table_metadata(table, requested: str, budget: list[int]) -> dict:
    ...
    for key in ("timePartitioning", "rangePartitioning"):
        if key in raw:
            if not isinstance(raw[key], dict) or not raw[key]:
                raise SchemaInspectionError("partition metadata is invalid")
            result[key] = raw[key]
...
def inspect_schema(bq, table_ids: list[str], *, allowed_tables: frozenset[str]) -> SchemaSnapshot:
    ...
    metadata = json.dumps({"version": 1, "tables": tables}, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
