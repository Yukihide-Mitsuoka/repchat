...
def _period_diagnostic(
    ...
    """Validate the generated period against the canonical contract."""
...
def _period_repair_guidance(
    ...
    """Render repair instructions from the canonical contract."""
...
def run_section(
    ...
    *,
    ...
    request = report.generation_request(section)
    ...
    if not sql:
        raise SectionExecutionError(
            "SQLが返りませんでした。共通分析契約または分析仕様を確認してください。"
        )
    ...
    if section.get("source_columns"):
        ...
        analysis_request = report.generation_request(section)
        repair_used = False
        while True:
            diagnostic = _dashboard_sql_diagnostic(
                section,
            ...
            if period_diagnostic and allow_period_repair:
                period_diagnostic += (
                    " 修正要件: " + _period_repair_guidance(policy)
                )
