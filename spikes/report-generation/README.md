...
`demo-live`は`127.0.0.1:8765`だけで待ち受け、ダッシュボード生成と単一グラフ生成を画面内で
切り替えます。各送信で同じ指標定義、SQL安全検査、20GiB上限を使用します。生成・検査・実行・描画の
段階、生成理由、SQL、結果、Vertex AI推定費用をEvidence基調の同じ画面に表示します。SQLは
...
単一グラフと各ダッシュボードカードのSQL右上にはコピー操作を置き、成功・失敗をボタンの表示と
...
グラフとの対応を記憶する必要はありません。宣言されたKPI・ファネル・時系列・Sankeyの結果形状と
...
許可します。根拠に無い数値や未登録panel IDは報告全体をfail closedで拒否します。旧形式の数字を
含まない文字列要約は互換入力として受理しますが、新しい生成schemaは根拠付き形式だけを返します。
...
「この結果から会議報告案を生成」を押すと、費用確認待ちであることを進行欄へ直ちに表示してから
...
途中段階で終了したセッションは段階別に注記します。範囲外depthはクラウド呼出し前に拒否します。
