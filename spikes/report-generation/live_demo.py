...
$("mode-dashboard").onclick=()=>selectMode("dashboard");$("mode-graph").onclick=()=>selectMode("graph");$("dataset-profile").onchange=()=>selectProfile($("dataset-profile").value);$("result-tab-chart").onclick=()=>selectResultTab("chart");$("result-tab-data").onclick=()=>selectResultTab("data");$("submit").onclick=()=>showCost("graph");$("dashboard-submit").onclick=()=>{currentAnswers={};currentPlan=null;pendingPlan=null;showCost("dashboard-plan")};$("plan-revise").onclick=()=>{collectAnswers();showCost("dashboard-plan")};$("plan-build").onclick=()=>{try{pendingPlan=selectedPlan();showCost("dashboard-build")}catch(e){$("dashboard-message").className="notice error";$("dashboard-message").textContent=e.message}};$("report-submit").onclick=()=>showCost("report");$("cancel-cost").onclick=()=>$("cost-dialog").close();$("confirm-cost").onclick=()=>pendingMode==="dashboard-plan"?runPlan():pendingMode==="dashboard-build"?runDashboard():pendingMode==="report"?runMeetingReport():runQuery();
