...
<p id="page-lead" class="lead">1つの具体的な分析依頼を、KPI・ファネル・時系列・回遊へ分解し、ひとつの画面へまとめます。</p></div>
...
<div id="dashboard-workspace"><section class="panel query-panel"><label for="dashboard-question">作りたいダッシュボード</label><textarea id="dashboard-question">2021年1月のECサイトについて、購入成果、ユーザーの定着とエンゲージメント、購入ファネル、日別の変動、主要なサイト回遊をまとめたダッシュボードを作って</textarea>
<p class="lead">この段階では、実測済みのECサイト分析6件へ自動分解します。目的からKPI候補を相談・提案する対話は次の開発段階です。</p><div class="actions"><button id="dashboard-submit">ダッシュボードを生成</button><span class="cost">6件のSQL生成と、最大12件のBigQuery読み取りを行います。</span></div></section>
<section class="panel" aria-labelledby="dashboard-progress-title"><div class="progress-head"><h2 id="dashboard-progress-title">ダッシュボード生成の進行状況</h2><span id="dashboard-status" class="status-pill">実行前</span></div><p id="dashboard-message" class="notice" aria-live="polite">依頼を確認し、生成ボタンを押してください。</p><div id="dashboard-plan" class="plan-list"><div class="plan-item"><strong>購入成果・定着</strong><span>意思決定に必要なKPI</span></div><div class="plan-item"><strong>ファネル・日別推移</strong><span>変化と減少箇所の説明</span></div><div class="plan-item"><strong>サイト回遊</strong><span>行動経路の診断</span></div></div></section>
<section id="dashboard-output" class="hidden"><div class="panel dashboard-head"><div><p class="eyebrow">Generated dashboard</p><h2 id="dashboard-title">月次ECサイト分析</h2><p class="lead">左上の成果KPIから、右下の行動診断へ読み進めます。</p></div><div><strong id="dashboard-cost">Vertex AI推定 ¥0</strong><p class="chart-caption">BigQuery利用料は別</p></div></div><div id="dashboard-grid" class="dashboard-grid"></div></section></div>
<div id="graph-workspace" class="hidden"><section class="panel query-panel"><label for="question">日本語の問い合わせ</label><textarea id="question">2021年1月のセッション数を流入チャネル（medium）別に、多い順で出して</textarea>
...
<section class="panel"><h2>BigQuery実行結果</h2><div id="chart" class="chart"></div></section><section class="panel"><h2>Vertex AIが生成したSQL</h2><pre id="sql" class="sql"></pre></section></section></div>
<p class="lead local-note">ローカルデモです。本番の認証・gate・executor・顧客Git配送は通りません。KPI相談はIssue #180、会議報告アシストはIssue #181の後続機能です。</p></main>
...
const $=id=>document.getElementById(id),stages=["generate","validate","execute","render"],dashboardPanels=new Map();let pendingMode="dashboard";document.querySelectorAll("[data-q]").forEach(b=>b.onclick=()=>{$("question").value=b.dataset.q});
...
function handleDashboard(e){if(e.type==="dashboard_plan"){$("dashboard-output").className="";$("dashboard-title").textContent=e.period+" 月次ECサイト分析";$("dashboard-grid").replaceChildren();dashboardPanels.clear();e.panels.forEach(createDashboardCard);$("dashboard-message").textContent=`${e.panels.length}件の分析へ分解しました。順番にSQLを生成します。`;return}const panel=dashboardPanels.get(e.panel_id);if(e.type==="stage"&&panel){panel.state.textContent=`${e.panel_index}/${e.panel_count} ${e.stage==="generate"?"SQL生成中":"BigQuery実行中"}`;$("dashboard-message").textContent=`${e.title}: ${e.message}`;return}if(e.type==="sql"&&panel){panel.sql.textContent=e.sql;panel.reason.textContent=e.reason;panel.state.textContent="SQL検査済み";return}if(e.type==="result"&&panel){graph(e,panel.chart);panel.data.replaceChildren(table(e.columns,e.rows));panel.verification.className=e.verification==="matched"?"notice":"notice warning";panel.verification.textContent=e.verification_label;panel.state.textContent="描画完了";return}if(e.type==="refusal"&&panel){panel.reason.textContent=e.reason;panel.verification.className="notice warning";panel.verification.textContent=`未定義のため停止: ${e.undefined_terms.join("、")}`;panel.state.textContent="停止";return}if(e.type==="dashboard_complete"){$("dashboard-status").textContent="完了";$("dashboard-cost").textContent=`Vertex AI推定 ¥${e.cost_jpy}`;$("dashboard-message").textContent=`${e.panel_count}件のSQL生成・実行・描画が完了しました。`;return}if(e.type==="error")throw new Error(e.message)}
async function stream(endpoint,question,eventHandler){const res=await fetch(endpoint,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({question})});if(!res.ok)throw new Error((await res.json()).error);const reader=res.body.getReader(),dec=new TextDecoder();let buf="";while(true){const{done,value}=await reader.read();buf+=dec.decode(value||new Uint8Array(),{stream:!done});const lines=buf.split("\n");buf=lines.pop();for(const line of lines)if(line)eventHandler(JSON.parse(line));if(done){if(buf.trim())eventHandler(JSON.parse(buf));break}}}
function selectMode(mode){const dashboard=mode==="dashboard";$("dashboard-workspace").className=dashboard?"":"hidden";$("graph-workspace").className=dashboard?"hidden":"";$("mode-dashboard").className=dashboard?"selected":"";$("mode-graph").className=dashboard?"":"selected";$("mode-dashboard").setAttribute("aria-pressed",String(dashboard));$("mode-graph").setAttribute("aria-pressed",String(!dashboard));$("page-title").textContent=dashboard?"日本語からダッシュボードを生成":"日本語からSQLとグラフを生成";$("page-lead").textContent=dashboard?"1つの具体的な分析依頼を、KPI・ファネル・時系列・回遊へ分解し、ひとつの画面へまとめます。":"質問からSQL生成・安全検査・BigQuery実行・可視化までを、ひとつの分析ワークスペースで確認できます。"}
function showCost(mode){const input=$(mode==="dashboard"?"dashboard-question":"question"),message=$(mode==="dashboard"?"dashboard-message":"message");if(!input.value.trim()){message.className="notice error";message.textContent="問い合わせを入力してください。";return}pendingMode=mode;const dashboard=mode==="dashboard";$("cost-description").textContent=dashboard?"このダッシュボードでは実際のVertex AIとBigQueryを複数回使用します。":"この質問では実際のVertex AIとBigQueryを使用します。";$("cost-vertex").textContent=dashboard?"Vertex AI 約¥1.2（SQL生成6回）":"Vertex AI 約¥0.2";$("cost-bigquery").textContent=dashboard?"BigQuery 最大240 GiB（20 GiB × 最大12クエリ、最大約¥228）":"BigQuery 最大40 GiB（20 GiB × 最大2クエリ、最大約¥38）";$("cost-total").textContent=dashboard?"合計最大約¥230":"合計最大約¥39";$("cost-dialog").showModal()}
$("mode-dashboard").onclick=()=>selectMode("dashboard");$("mode-graph").onclick=()=>selectMode("graph");$("submit").onclick=()=>showCost("graph");$("dashboard-submit").onclick=()=>showCost("dashboard");$("cancel-cost").onclick=()=>$("cost-dialog").close();$("confirm-cost").onclick=()=>pendingMode==="dashboard"?runDashboard():runQuery();
async function runQuery(){$("cost-dialog").close();const q=$("question").value.trim();$("submit").disabled=true;$("run-status").textContent="処理中";$("output").className="hidden";stage("generate");$("message").className="notice";$("message").textContent="Vertex AIへ問い合わせています。";try{await stream("/api/query",q,handle)}catch(e){$("message").className="notice error";$("message").textContent=e.message;finish("エラー")}finally{$("submit").disabled=false}}
async function runDashboard(){$("cost-dialog").close();const q=$("dashboard-question").value.trim();$("dashboard-submit").disabled=true;$("dashboard-status").textContent="処理中";$("dashboard-output").className="hidden";$("dashboard-message").className="notice";$("dashboard-message").textContent="分析を分解しています。";try{await stream("/api/dashboard",q,handleDashboard)}catch(e){$("dashboard-message").className="notice error";$("dashboard-message").textContent=e.message;$("dashboard-status").textContent="エラー"}finally{$("dashboard-submit").disabled=false}}
</script></body></html>"""
