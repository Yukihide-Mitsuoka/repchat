...
function handleMeetingReport(e){if(e.type==="report_stage"){selectWorkspace("report");$("dashboard-status").textContent="報告案作成中";$("workspace-state").textContent="生成中";$("dashboard-message").textContent=e.message}else if(e.type==="meeting_report"){renderMeetingReport(e);$("dashboard-status").textContent="要承認";selectWorkspace("report")}else if(e.type==="error")throw new Error(e.message)}
...
async function runDashboard(){$("cost-dialog").close();selectWorkspace("build");const q=$("dashboard-question").value.trim();$("plan-build").disabled=true;dashboardStage("build");$("dashboard-status").textContent="build中";$("dashboard-output").className="hidden";$("report-output").className="panel hidden report-home";$("report-empty").className="panel empty-state";$("report-submit").className="hidden";latestBuildRevision=null;$("dashboard-message").className="notice";$("dashboard-message").textContent="確定した分析仕様をfreezeし、buildを開始します。";try{await stream("/api/dashboard",q,handleDashboard,"ga4",{analysis_plan:pendingPlan})}catch(e){$("dashboard-message").className="notice error";$("dashboard-message").textContent=e.message;$("dashboard-status").textContent="エラー"}finally{$("plan-build").disabled=false}}
async function runMeetingReport(){$("cost-dialog").close();selectWorkspace("report");const q=$("dashboard-question").value.trim();$("report-submit").disabled=true;$("dashboard-status").textContent="報告案作成中";$("workspace-state").textContent="生成中";try{await stream("/api/report",q,handleMeetingReport,"ga4",{build_revision:latestBuildRevision})}catch(e){$("dashboard-message").className="notice error";$("dashboard-message").textContent=e.message;$("dashboard-status").textContent="エラー";$("workspace-state").textContent="エラー"}finally{$("report-submit").disabled=false}}
</script></body></html>"""
