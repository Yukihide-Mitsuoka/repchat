...
async function stream(endpoint,question,eventHandler,profile="ga4"){const res=await fetch(endpoint,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({question,profile})});if(!res.ok)throw new Error((await res.json()).error);const reader=res.body.getReader(),dec=new TextDecoder();let buf="";while(true){const{done,value}=await reader.read();buf+=dec.decode(value||new Uint8Array(),{stream:!done});const lines=buf.split("\n");buf=lines.pop();for(const line of lines)if(line)eventHandler(JSON.parse(line));if(done){if(buf.trim())eventHandler(JSON.parse(buf));break}}}
...
function showCost(mode){const input=$(mode==="dashboard"?"dashboard-question":"question"),message=$(mode==="dashboard"?"dashboard-message":"message");if(!input.value.trim()){message.className="notice error";message.textContent="問い合わせを入力してください。";return}pendingMode=mode;const dashboard=mode==="dashboard",bitcoin=!dashboard&&$("dataset-profile").value==="bitcoin";$("cost-description").textContent=dashboard?"このダッシュボードでは実際のVertex AIとBigQueryを複数回使用します。":"この質問では実際のVertex AIとBigQueryを使用します。";$("cost-vertex").textContent=dashboard?"Vertex AI 約¥1.2（SQL生成6回）":"Vertex AI 約¥0.2";$("cost-bigquery").textContent=dashboard?"BigQuery 最大240 GiB（20 GiB × 最大12クエリ、最大約¥228）":bitcoin?"BigQuery dry run 約2.91 GiB（上限20 GiB・参照値照合なし、最大約¥19）":"BigQuery 最大40 GiB（20 GiB × 最大2クエリ、最大約¥38）";$("cost-total").textContent=dashboard?"合計最大約¥230":bitcoin?"通常約¥4・最大約¥20":"合計最大約¥39";$("cost-dialog").showModal()}
$("mode-dashboard").onclick=()=>selectMode("dashboard");$("mode-graph").onclick=()=>selectMode("graph");$("dataset-profile").onchange=()=>selectProfile($("dataset-profile").value);$("result-tab-chart").onclick=()=>selectResultTab("chart");$("result-tab-data").onclick=()=>selectResultTab("data");$("submit").onclick=()=>showCost("graph");$("dashboard-submit").onclick=()=>showCost("dashboard");$("cancel-cost").onclick=()=>$("cost-dialog").close();$("confirm-cost").onclick=()=>pendingMode==="dashboard"?runDashboard():runQuery();
