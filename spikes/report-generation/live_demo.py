...
.chart .chart-axis-title{font-size:11px;font-weight:700}.chart .chart-axis-value{font-size:10px}.chart .sankey-stage{font-size:12px;font-weight:700;fill:#344054}.chart .sankey-link{outline:none}.chart .sankey-link:hover,.chart .sankey-link:focus{stroke-opacity:.9}.sankey-detail{min-height:18px;color:#344054}.sankey-terminal{border-left:3px solid #4b84b4;padding-left:9px}
...
const metricUnits=__METRIC_UNITS__;function metricUnit(column){const label=String(column),name=Object.keys(metricUnits).sort((a,b)=>b.length-a.length).find(candidate=>label.includes(candidate));return name?metricUnits[name]:""}function metricAxisTitle(column){const unit=metricUnit(column);return unit?`${column} (${unit})`:column}function chartValue(value,column="",forceInteger=false){if(value===null||value===undefined||value==="")return "—";const number=typeof value==="number"?value:Number(value);if(!Number.isFinite(number))return String(value);const key=String(column).toLowerCase(),countLike=forceInteger||/(count|sessions|users|purchases|pageviews|views|件数|セッション|ユーザー数|閲覧数|取引数)/.test(key);return new Intl.NumberFormat("ja-JP",{maximumFractionDigits:countLike?0:2}).format(number)}
...
if([...groups.values()].some(names=>names.length>maxPaths)){detail.textContent=`1段階に表示できるページは上位${maxPaths}件までです。`;return}
...
function timeAxisFrame(svg,r,scales,w,h,plot){
scales.forEach((scale,seriesIndex)=>{const left=seriesIndex%2===0,rank=Math.floor(seriesIndex/2),axisX=left?70+rank*66:w-70-rank*66,color=chartPalette[seriesIndex%chartPalette.length],title=node("text",{x:axisX,y:14,class:"chart-axis-title",fill:color,"text-anchor":"middle"});title.textContent=metricAxisTitle(r.columns[seriesIndex+1]);svg.append(title,node("line",{x1:axisX,y1:plot.top,x2:axisX,y2:plot.bottom,stroke:color,"stroke-opacity":.55}));[scale.max,(scale.max+scale.min)/2,scale.min].forEach((tickValue,tickIndex)=>{const y=plot.top+tickIndex*(plot.bottom-plot.top)/2;if(seriesIndex===0)svg.appendChild(node("line",{x1:plot.left,y1:y,x2:plot.right,y2:y,stroke:"#e4e7ec"}));const label=node("text",{x:left?axisX-6:axisX+6,y:y+4,class:"chart-axis-value",fill:color,"text-anchor":left?"end":"start"});label.textContent=chartValue(tickValue,r.columns[seriesIndex+1]);svg.append(label)})});if(r.rows.length){const first=node("text",{x:plot.left,y:h-25,"text-anchor":"start"}),last=node("text",{x:plot.right,y:h-25,"text-anchor":"end"});first.textContent=String(r.rows[0][0]);last.textContent=String(r.rows[r.rows.length-1][0]);svg.append(first,last)}}
...
if(r.visualization==="area"){areaChart(r,box,false);return}
if(r.visualization==="stacked_area"){areaChart(r,box,true);return}
if(r.visualization==="histogram"){histogramChart(r,box);return}
if(r.visualization==="donut"){donutChart(r,box);return}
if(r.visualization==="calendar_heatmap"){calendarHeatmapChart(r,box);return}
...
function setComposerSubmitIcon(stopping){const svg=node("svg",{viewBox:"0 0 16 16","aria-hidden":"true",focusable:"false"});if(stopping)svg.append(node("rect",{x:5,y:5,width:6,height:6,rx:1,fill:"currentColor"}));else svg.append(node("path",{d:"M8 12V4M8 4L4.75 7.25M8 4L11.25 7.25",fill:"none",stroke:"currentColor","stroke-width":1.8,"stroke-linecap":"round","stroke-linejoin":"round"}));$("composer-submit").replaceChildren(svg)}
function startActiveRequest(){if(activeRequest)throw new Error("実行中の処理を停止してから再送してください。");const random=Math.floor(Math.random()*0xffffffff).toString(16).padStart(8,"0"),operation={requestId:`request-${Date.now()}-${random}`,controller:new AbortController(),stopping:false};activeRequest=operation;$("analysis-composer").setAttribute("aria-busy","true");setComposerSubmitIcon(true);$("composer-submit").setAttribute("aria-label","実行中の処理を停止");$("composer-input").disabled=true;$("composer-profile").disabled=true;document.querySelectorAll("[data-composer-action]").forEach(button=>button.disabled=true);return operation}
function finishActiveRequest(operation,force=false){if(activeRequest!==operation||operation.stopping&&!force)return;activeRequest=null;$("analysis-composer").removeAttribute("aria-busy");$("composer-submit").disabled=false;setComposerSubmitIcon(false);$("composer-submit").setAttribute("aria-label","分析指示を送信");$("composer-input").disabled=false;$("composer-profile").disabled=false;document.querySelectorAll("[data-composer-action]").forEach(button=>button.disabled=false)}
...
def planned_analysis_section(panel: dict, section_id: str | None = None) -> dict:
    ...
    if chart == "scorecard":
        ...
        section["generation_requirements"] = [
            ...
            f"指定した最終ページへ到達した完全な経路を集計し、全ページ列を安定した順序条件へ含めてmetric_valueの上位{MAX_SANKEY_PATHS}経路を先に選ぶ",
            "上位経路を選んだ後で、各経路をsourceとtargetの隣接edgeへ変換する",
