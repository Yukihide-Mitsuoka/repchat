...
- 2種類以上のschemaを同じ評価に含め、各caseを3回以上反復する。
...
- schema別の結果一致率90%以上、安全違反・意味誤り0件を合格条件にする。

bundleには`version`、固定下限以上の`thresholds`、2件以上の`schemas`を記録します。schemaごとのcaseは質問、
参照SQL、期待行、行順序、review記録、3件以上のrunを持ちます。runには同一pipelineのfingerprint、実際の
...
証拠ではありません。独立review済みfixtureの作成と実値照合は未完了です。有料評価は対象と費用について
オーナー承認を得た後だけ実行します。
