...
- **SoR（真実の置き場）＝ Postgres**（原則D）。理由:
  - **整合性要求は非対称**。付与の遅延は不便で済むが、**剥奪の遅延は「消したはずの権限が残る」＝セキュリティ事故**。ChatChartは剥奪の即時性を `epoch＋デニーリスト` で保証しており、これは SoR が一貫している前提。結果整合のNoSQLをSoRにすると「いつ剥奪が効いたか」が曖昧になり、この保険とキャッシュ失効判断が噛み合わない。
...
7. **分析データの供給元**：ChatChartがホストするか、パートナー既存のBigQuery/ウェアハウスに接続するか（`datasource` 抽象はどちらも許容）。パートナー折衝で確定。
