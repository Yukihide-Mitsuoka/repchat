...
| 組織コンテキスト | [用語集](../glossary.md#用語)で定義した、tenantまたは分析対象組織の承認済み分析前提。指標定義、分析レシピ、user表示嗜好を含めない |
...
| C-1 | 制約 | #188の対象非依存品質境界と#179/#180のrevision契約が安定するまで製品実装を開始しない | Phase 0の文書化だけを行う |
...
| C-6 | 制約 | 要件具体化とローカルの未検証プロトタイプは進められるが、永続メモリーの製品実装開始条件はC-1を維持する | プロトタイプの結果を本番統合済みまたは検証済み能力と表明しない |
...
- **目的:** 顧客組織の事業前提と、期待に合わなかった分析から得た修正を、誤適用や無断更新なしに次回の分析へ反映する。
...
- **成功指標:**
  ...
  | 組織コンテキストの由来と鮮度 | 100% | 有効revisionごとにowner、source、根拠、適用日、見直し日、版を確認 |
...
- **対象外:** fine-tuning、生の全会話RAG、顧客間の暗黙学習、結果値・生成SQLの記憶、任意深度の組織tree、対象非依存品質境界の確定前に個別意味定義を製品経路へ導入すること。
...
| ID | 要件 | 目的 | 優先度 | 根拠 |
|----|------|------|--------|------|
| FR-001 | 分析方針メモリーをsystem・security、組織コンテキスト、分析レシピ、user、sessionの区分と、tenant・分析対象のscopeで保存する | 誤適用0件 | Must | 顧客固有要望と個人嗜好の分離 |
...
| FR-006 | schema版、組織コンテキストrevision、指標定義版が非互換なら既存方針を適用せず、再確認または拒否する | 誤分析防止 | Must | ADR-0013、Issue #188 |
...
| FR-010 | 組織コンテキスト、指標定義、security制約は権限者の承認前に有効化しない | 未承認有効化0件 | Must | 影響範囲に応じたauthority |
...
2. 利用者が「この顧客では売上を税込で見たい」と修正した場合、今回の表示を直して永続化scopeを質問し、組織コンテキストまたは指標定義なら承認待ち候補にする。
...
| 組織コンテキスト（tenant scope） | tenant組織の事業特性、共通目標、会計年度、禁止する解釈、承認workflow | tenant管理者 | 方針が許可した範囲だけ |
| 組織コンテキスト（分析対象scope） | 顧客組織の事業特性、収益構造、戦略目標、目標階層、意思決定周期、事業制約、固有用語 | 編集者または管理者 | session override可否を方針で定義 |
...
`tenant_id + analysis_subject_id/workspace + datasource/schema version + organization_context_revision + analysis purpose/family + audience + metric_definition_version + recipe_version`
...
- schema・組織コンテキスト・指標定義の互換性検証に失敗した場合は`incompatible`として適用対象から外す。
...
|-------------|------|------|-----|--------|
| Issue #180 分析仕様 | 双方向 | analysis specification revision、組織コンテキストrevision、fingerprint、実行時パラメータ | 未定 | 組織コンテキスト依存の提案を停止し、明示確認へfallback |
...
| AC-5 | schema版、組織コンテキストrevision、指標定義版が非互換なら方針を適用せず、有料実行前に止まる | FR-006 | compatibility test |
...
| Phase 1 — 手動で統制された方針と契約 | FR-001、004〜008、010、012〜024。組織コンテキスト、任意org unit、用途別compiler、表示、手動作成・承認・取消。embeddingと自動昇格なし | #188の対象非依存品質境界、#179完了、#180でanalysis specification revision契約を確定、ADR-0018 accepted |
...
Phase 1の実装Issueは、#188の対象非依存品質境界と#179/#180のrevision契約が確定した後に作る。
Issue #180の最初の縦串はsession-onlyでも進められるが、永続的な個別化の前にPhase 1を完了する。
Issue #181も同じpolicy revisionを参照する。
