...
| NAV-013 | 左右paneと中央paneは1px境界線で隣接し、resize操作のための余白をlayoutへ追加しない。pointer用の8px hit areaは境界線を中心に両paneへ重ねる。pane内のheader／footer区切り線はpaneのcontent paddingを越えて縦境界へ接続する | Must |
