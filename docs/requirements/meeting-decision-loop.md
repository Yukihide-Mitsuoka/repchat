...
| C-1 | 制約 | #188の対象非依存品質境界と#179/#180のrevision契約が安定するまで製品実装しない | 今回は要件記録だけを行う |
...
- **対象外:** 会議録音、音声からの自動決定、AIによる自動承認、任意式の実行、根拠のない予測、
  外部タスク管理サービスとの同期、根拠revision経路が安定する前の製品実装。
...
| Phase 1 — 意思決定中心の会議パック | FR-001〜006、010〜012。1分報告、最大3論点、検証可能scenario、根拠Q&A | #188の対象非依存品質境界と#179/#180のrevision・閲覧契約が安定し、Issue #181の根拠付き報告が製品経路へ接続 |
