...
[`sql_generation.py`](../../spikes/report-generation/sql_generation.py)、SQL担当AIへ渡す共通契約から導出したschema・意味候補・期間・汎用規則は
[`analysis_contract_context.py`](../../spikes/report-generation/analysis_contract_context.py)、planner出力の目的・期間・確認事項・revisionと文字列・軸・指標・可視化形状の検証は
