...
| 分類 | 検出箇所 | 判定・扱い |
|---|---|---|
| 対象registryと既定値 | `data_source_profiles.py`、`live_http_validation.py`、`live_engine.py`、`analysis_workflows.py`、`analysis_dashboard_plan.py`、`analysis_planner.py`、`live_contracts.py` | GA4／Bitcoinの登録済み対象だけを受け付け、複数箇所でGA4を既定値にする実行時固有処理。共通の自動発見pipelineへ置換して削除する。旧実サービス検証runnerは削除済み |
...
| UIと補助実行経路 | `live_ui_base.py`、`live_ui_interactions.py`、`tenant_serve.py`、`evidence_components.py` | 固定の対象選択肢・文言・問い合わせ・source名を持つ。旧ライブデモ入口は削除済み。残るUI補助moduleも共通runtimeへ移行せず、固有知識を持つものは削除する |
...
| 5 | UI・成果物を中立化 | `live_ui_base.py`、`live_ui_interactions.py`、`evidence_components.py`、`tenant_serve.py`、`visualization_contracts.py`、`visualization_sections.py` | 旧ライブデモ入口は削除済み。残る固定のGA4／Bitcoin選択肢、期間、例文、metric語彙、source名、問い合わせを削除する。`event_date`を中立なtemporal roleへ置換し、可視化は契約が対応する意味roleを持つ場合だけ選択する |
