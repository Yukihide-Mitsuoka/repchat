...
| 手書きschema・意味・期間・SQL補正 | `sql_generation.py`、`sql_contract_validation.py`、`bigquery_execution.py`、`run_report.py` | 対象別profile moduleはPR #722、固定schema・指標promptと手動metric資産はPR #723で削除済み。他の対象別期間、partition、識別子、URL処理、datasetは例外を増やさず順次削除する |
| UIと補助実行経路 | `tenant_serve.py` | 固定の対象選択肢・文言を持つ。旧ライブデモ入口とUI補助moduleは削除済み。未参照の固定Evidence成果物出力は現在の変更で削除する。残るUI補助moduleも共通runtimeへ移行せず、固有知識を持つものは削除する |
...
| 5 | UI・成果物を中立化 | `tenant_serve.py`、`visualization_contracts.py`、`visualization_sections.py` | 旧ライブデモ入口とUI payloadは削除済み。未参照の固定Evidence成果物出力は現在の変更で削除する。残る固定のmetric語彙、source名、問い合わせを除去する。`event_date`を中立なtemporal roleへ置換し、可視化は契約が対応する意味roleを持つ場合だけ選択する |
| 6 | 旧実装を物理削除（進行中） | `run_report.py`の旧export等 | `dashboard_build.py`、registry、対象別profile moduleはPR #720〜#722、固定schema・手動metric資産はPR #723で削除済み。固定Evidence成果物出力と対応する旧exportは現在の変更で削除する。残る対象別処理を順次除去し、互換目的のadapter、feature flag、隠し設定を残さずruntime inventoryのallowlistを空にする |
