...
現在は[PR #751](https://github.com/Yukihide-Mitsuoka/repchat/pull/751)で、runtimeが生成したcanonical analysis contract
artifactをschema／caseごとに受け取り、内容から再計算したfingerprintを全runへ照合する境界を実装中です。contract内の
schema observationも対応するscope snapshotへ照合し、contract本文はevidenceへ複製しません。
