...
[PR #869](https://github.com/Yukihide-Mitsuoka/repchat/pull/869)は2026-09-26にmerge済みです。診断側の契約artifact・scope snapshot・独立review記録を照合します。次のsliceはreview記録を両artifact bytesへ固定し、再取得scopeの一致後だけreview済み契約を返す評価専用preflightを追加します。全段階への接続と実runの利用証明は未完了です。
...
| 2 | [#791](https://github.com/Yukihide-Mitsuoka/repchat/issues/791)のbaseline主導SQL精度改善 | PR #867〜#869はmerge済み。診断contractの事前照合と評価専用preflightを追加中。全段階での実行時利用証明、対抗fixture、baselineは未完了。再現した原因に対応する対象非依存の改善だけを比較する |
