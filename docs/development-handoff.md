...
| 3 | planner・SQL・検査を契約だけへ接続（PR #687、#689、#691 merge済み、PR #693 review中） | `analysis_planner.py`、`analysis_workflows.py`、`sql_generation.py`、`sql_contract_validation.py`、`bigquery_execution.py`、`section_execution.py` | PR #687ではexact table scopeと実行上限、PR #689では期間policy、PR #691では型別`WHERE`制約と修正指示をcanonical contractから導出し、契約付き経路で旧profileの期間callbackを呼ばない。PR #693ではcontract metadataをtable、完全field path、標準型、mode、継承repeated・policy tagから成る汎用field policyへflattenし、期間fieldの完全path・型・制御も照合する。次はこのpolicyを一般列のalias・identifier・UNNEST検査へ接続し、結果形状を導出する。最後にplannerとSQL生成からlegacy profile入力、URL、event等の特殊補正を削除する |
