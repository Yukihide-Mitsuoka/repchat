...
[Issue #812](https://github.com/Yukihide-Mitsuoka/repchat/issues/812)では次のsliceとして、local SQL validation、
BigQuery dry run、BigQuery executionの既知診断だけを失敗runへ変換し、未知例外と不変条件違反を伝播させます。
次はpreflight、planning、SQL生成の分類とprovider／infrastructure failureのrun契約です。
...
| 順序 | 作業 | 完了条件・次への移行条件 |
|---:|---|---|
| 1 | [#788](https://github.com/Yukihide-Mitsuoka/repchat/issues/788)の実評価前の評価契約強化 | 型付きSQL診断は#796／#799／#802／#804、case／capability／描画の合格gateは#807で実装。[#809](https://github.com/Yukihide-Mitsuoka/repchat/issues/809)／[PR #810](https://github.com/Yukihide-Mitsuoka/repchat/pull/810)で結果検証・描画の未知例外伝播を実装し、merge済み。[#812](https://github.com/Yukihide-Mitsuoka/repchat/issues/812)でlocal SQL validation、dry run、executionにも同じ境界を追加する。次にpreflight、planning、SQL生成の分類とprovider／infrastructure failureのrun契約を実装する。attempt状態機械の全面再設計は行わない。完了後に公式fixtureと実行commandへ進む |
