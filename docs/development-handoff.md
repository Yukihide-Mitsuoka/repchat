...
次は診断側の契約artifact・scope snapshot・独立review記録をevidenceに結び付けますが、実際のcontract使用はfile照合だけでは未証明です。
...
| 2 | [#791](https://github.com/Yukihide-Mitsuoka/repchat/issues/791)のbaseline主導SQL精度改善 | [PR #867](https://github.com/Yukihide-Mitsuoka/repchat/pull/867)のpost-run不一致原因reviewと[PR #868](https://github.com/Yukihide-Mitsuoka/repchat/pull/868)の通常・診断evidence offline比較はmerge済み。診断contractの独立review記録・artifactを照合する作業中。契約の実行時利用証明、対抗fixture、baselineは未完了。再現した原因に対応する対象非依存の改善だけを比較する |
