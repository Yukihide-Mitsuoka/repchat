...
| 手書きschema・意味・期間・SQL補正 | `sql_generation.py`、`sql_contract_validation.py`、`bigquery_execution.py`、`run_report.py` | 対象別profile moduleはPR #722、固定schema・指標promptと手動metric資産はPR #723、固定Evidence出力はPR #725で削除済み。現在の変更でSQL実行の固定datasetと20GiB fallbackを撤去する。残るURL等の特殊補正は例外を増やさず順次削除する |
...
| 6 | 旧実装を物理削除（進行中） | `run_report.py`の旧export等 | `dashboard_build.py`、registry、対象別profile moduleはPR #720〜#722、固定schema・手動metric資産はPR #723、固定Evidence成果物出力と対応する旧exportはPR #725で削除済み。現在の変更で固定dataset・20GiB上限のexportも撤去する。残る対象別処理を順次除去し、互換目的のadapter、feature flag、隠し設定を残さずruntime inventoryのallowlistを空にする |
