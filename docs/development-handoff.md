...
合格不能にするrecordings／evidence契約を実装し、CI確認中です。次は各stageから伝播する安全な基盤例外を
この契約へ接続します。
