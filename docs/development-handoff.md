...
| 3 | planner・SQL・検査を契約だけへ接続（PR #687、#689、#691、#693、#695、#696 merge済み。PR #697 review中） | `analysis_planner.py`、`analysis_workflows.py`、`sql_generation.py`、`sql_contract_validation.py`、`bigquery_execution.py`、`section_execution.py` | PR #687ではexact table scopeと実行上限、PR #689では期間policy、PR #691では型別`WHERE`制約と修正指示、PR #693では汎用field policy、PR #695ではphysical table alias・nested identifier・UNNEST検査、PR #696では結果roleとdry run・実行結果の形状検査を実装した。PR #697ではdashboard計画と分析相談のVertex AI request・response schema seedからprofile IDを除去する。次は保存planのprofile bindingとSQL生成のlegacy profile入力、URL等の特殊補正を削除する |
