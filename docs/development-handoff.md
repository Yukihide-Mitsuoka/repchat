...
| 3 | planner・SQL・検査を契約だけへ接続（PR #687 merge済み、`codex/188-contract-period-validation`で続行中） | `analysis_planner.py`、`analysis_workflows.py`、`sql_generation.py`、`sql_contract_validation.py`、`bigquery_execution.py`、`section_execution.py` | PR #687ではcanonical contractからexact table scope、date shardの物理table、query bytes上限、結果行上限を導出し、契約付き全実行へ必須dry runと同じ上限を適用した。現在は比較期間を含むscan範囲、timezone、業務時刻とpartitionのfield path・標準型をcontractから汎用policyへ導出している。次のPRでこのpolicyをSQL検査へ接続し、その後に結果形状、identifier、nested/repeated検査を導出する。最後にplannerとSQL生成からlegacy profile入力、URL、event、Bitcoin等の特殊補正を削除する |
