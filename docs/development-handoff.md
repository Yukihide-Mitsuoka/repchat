...
| 2 | 共通分析契約の自動生成（前半PR #679、merge済み） | `analysis_contract.py`、`analysis_contract_context.py`、共通compiler | 構造化validatorはmerge済み。現在はdiscovery snapshotのfingerprint・policy・schema整合を再検証し、fieldを不透明tokenへ束縛する生成入力を小さいPRで追加する。次に同じ入力から構造化候補を生成してvalidatorへ接続し、自然言語期間をISO閉区間へ固定する。手動意味定義、対象別period parser、識別子補正を使わない |
