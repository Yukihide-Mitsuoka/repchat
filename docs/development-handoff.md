...
| 2 | [#791](https://github.com/Yukihide-Mitsuoka/repchat/issues/791)のbaseline主導SQL精度改善 | [PR #867](https://github.com/Yukihide-Mitsuoka/repchat/pull/867)のpost-run不一致原因reviewはmerge済み。通常・診断用evidenceの条件を照合するoffline比較を進めるが、review済みcontractの生成・利用証明は別途必要。続いて対抗fixture、baselineを固定する。再現した原因に対応する対象非依存の改善だけを比較する |
