...
| 完了したデモ調整 | [#355 composer and dashboard row resize](https://github.com/Yukihide-Mitsuoka/repchat/issues/355)／[PR #356](https://github.com/Yukihide-Mitsuoka/repchat/pull/356)。composerは中央列へ追従しつつ960pxで上限、22px角丸とする。dashboardは同じ行の全境界を操作可能にし、隣接cardだけを連動させる。左paneは一行title、hover／focus marquee、16px icon列、4px gap／paddingへ圧縮する。順序変更、自由配置、永続化はしない | [分析ワークスペースUI要件](requirements/analysis-workspace-ui.md)、[デモ手順](demo.md)、`live_demo.py`、`live-demo.test.ts` |
...
1. **現在:** [Issue #357](https://github.com/Yukihide-Mitsuoka/repchat/issues/357)で、探索的な問いを分析候補の相談へrouteし、候補選択後も自動実行しない二段階UXを固定応答で検証する。実Vertex AI・BigQueryは実行しない。
