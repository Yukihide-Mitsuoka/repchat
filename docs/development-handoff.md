...
| 3 | planner・SQL・検査を契約だけへ接続（PR #687、#689 merge済み、PR #691 review中） | `analysis_planner.py`、`analysis_workflows.py`、`sql_generation.py`、`sql_contract_validation.py`、`bigquery_execution.py`、`section_execution.py` | PR #687ではcanonical contractからexact table scope、date shardの物理table、query bytes上限、結果行上限を導出し、契約付き全実行へ必須dry runと同じ上限を適用した。PR #689では比較期間を含むscan範囲、timezone、業務時刻とpartitionのfield path・標準型をcontractから汎用policyへ導出した。PR #691ではこのpolicyからDATE_SHARD、DATE、DATETIME、TIMESTAMPの`WHERE`制約と修正指示を汎用生成し、契約付き経路で旧profileの期間callbackを呼ばない。次は結果形状、identifier、nested/repeated検査を導出し、最後にplannerとSQL生成からlegacy profile入力、URL、event等の特殊補正を削除する |
