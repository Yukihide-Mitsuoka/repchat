...
| 項目 | 現在地 |
|------|--------|
| 作業 | [Issue #279](https://github.com/Yukihide-Mitsuoka/repchat/issues/279) — PR #278はmerge済み。AI推奨を直接確定した後、正常な確定計画が共通4 KiB本文上限で拒否される問題を`fix/279-dashboard-body-limit`で修正中。ダッシュボードだけ16 KiBの有限上限とし、送信パネルを`id`・`reason`へ縮小した回帰テストは成功 |
...
| 条件 | 次の作業 | 先に読む正本 |
|------|----------|--------------|
| 現在のデモ阻害 | [#279 確定計画が共通4 KiB本文上限で拒否される](https://github.com/Yukihide-Mitsuoka/repchat/issues/279)。HTTP境界のfailing-first testと修正は完了し、専用PR作成、CI確認、再起動が残る | #279、`live_demo.py`、live-demo tests |
...
1. **現在:** [Issue #279](https://github.com/Yukihide-Mitsuoka/repchat/issues/279)の専用PRを作成し、CIを確認する。
