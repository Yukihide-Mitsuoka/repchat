...
実行前に検証して全計画attemptを共通preflightへ渡し、失敗runをmanifestのpipeline fingerprintへ固定する境界を実装しています。
