...
| 3 | planner・SQL・検査を契約だけへ接続（PR #687、#689、#691、#693 merge済み。PR #695 review中） | `analysis_planner.py`、`analysis_workflows.py`、`sql_generation.py`、`sql_contract_validation.py`、`bigquery_execution.py`、`section_execution.py` | PR #687ではexact table scopeと実行上限、PR #689では期間policy、PR #691では型別`WHERE`制約と修正指示、PR #693では完全field path・型・mode・継承制御から成る汎用field policyをcanonical contractから導出した。PR #695ではこのpolicyをphysical table alias、nested identifier、UNNESTの実行前検査へ接続する。次は結果形状を導出し、plannerとSQL生成からlegacy profile入力、URL、event等の特殊補正を削除する |
