...
| 項目 | 現在地 |
|------|--------|
| 作業 | [Issue #279](https://github.com/Yukihide-Mitsuoka/repchat/issues/279) / [PR #280](https://github.com/Yukihide-Mitsuoka/repchat/pull/280) — PR #278はmerge済み。AI推奨を直接確定した後、正常な確定計画が共通4 KiB本文上限で拒否される問題を修正。ダッシュボードだけ16 KiBの有限上限とし、送信パネルを`id`・`reason`へ縮小してCI確認中 |
| デモ実行状態 | `http://127.0.0.1:8765/`のプロセスは#279修正前のコードで起動中。実planner相談は成功したが、build POSTは400で停止し、Vertex AI・BigQuery buildは開始していない。#279修正後に再起動する |
...
| 条件 | 次の作業 | 先に読む正本 |
|------|----------|--------------|
| 現在のデモ阻害 | [#279 確定計画が共通4 KiB本文上限で拒否される](https://github.com/Yukihide-Mitsuoka/repchat/issues/279) / [PR #280](https://github.com/Yukihide-Mitsuoka/repchat/pull/280)。HTTP境界のfailing-first testと修正は完了し、CI確認と再起動が残る | #279、`live_demo.py`、live-demo tests |
...
1. **現在:** [PR #280](https://github.com/Yukihide-Mitsuoka/repchat/pull/280)のCIを確認してmergeする。
2. **次:** #279の作業branchからデモを再起動し、HTTP 200と修正版HTMLを無料で確認する。実相談・buildの再実行は費用を別途提示して承認を得る。
