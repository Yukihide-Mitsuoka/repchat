...
[Issue #820](https://github.com/Yukihide-Mitsuoka/repchat/issues/820)では次のsliceとして、成功、品質失敗、
provider／infrastructure failureを閉じたrun種別へ分離し、基盤障害を品質率の分母から除外しつつ評価を
合格不能にするrecordings／evidence契約を実装中です。次は各stageから伝播する安全な基盤例外をこの契約へ接続します。
