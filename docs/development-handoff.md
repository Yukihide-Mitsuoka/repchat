...
現在の受入条件と進行状況は[Issue #605](https://github.com/Yukihide-Mitsuoka/repchat/issues/605)を参照してください。
対象は`live-demo.test.ts`に混在するchart capabilityの対応表契約4テストです。AIが選べるchartからSQL出力、結果検証、ブラウザrendererへ到達する網羅性と、標準rendererのECharts option生成を`live-demo-chart-contracts.test.ts`へ移します。複数のtest fileが使うrenderer source読込処理は既存の`live-demo-test-helpers.ts`へ集約します。テスト名・fixture・assertion・各テスト内の呼出し順序は変更せず、移動前後の本文一致を確認しました。製品コード、chart対応種別、SQL・結果shape、timeout、retry、skipも変更しません。移動元は1,652行から1,344行になり、専用ファイルは296行です。残るseries variant・map・table・成果物遷移の検証は別PRで責務単位に整理します。
基準はworkspaceレイアウト契約を分離した[PR #604](https://github.com/Yukihide-Mitsuoka/repchat/pull/604)を取り込んだmainです。変更前後の`make test-unit`、変更後の`make test`と`make coverage`は356件成功し、format・lintも成功しました。Nodeのcoverageはline 93.66%、branch 85.26%、function 87.95%で基準の93.63%／85.22%／87.84%から低下していません。Python subprocessの行coverageと実ブラウザ目視は対象外です。
