...
現在は、runtimeが生成したcanonical scope snapshot artifactをschema IDごとに受け取り、その内容のSHA-256をfixtureと
全runのscope snapshot fingerprintへ一対一で照合する作業を進めています。生のschema metadata、bounded value profile、
取得時刻はevidenceへ複製しません。
