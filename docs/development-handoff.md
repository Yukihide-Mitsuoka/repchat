...
ローカル検証と必須CI checksは成功し、2026-09-22にmerge済みです。
[Issue #814](https://github.com/Yukihide-Mitsuoka/repchat/issues/814)では、planningの明示的な出力拒否と
SQL生成の`SQLGenerationError`・不正応答だけを品質失敗へ変換し、runner例外、成功状態の必須field欠落、
section生成・費用計算・複製処理の不変条件違反を伝播させます。
