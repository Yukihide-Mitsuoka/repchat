...
| 3 | planner・SQL・検査を契約だけへ接続（PR #687、#689、#691、#693、#695 merge済み。PR #696 review中） | `analysis_planner.py`、`analysis_workflows.py`、`sql_generation.py`、`sql_contract_validation.py`、`bigquery_execution.py`、`section_execution.py` | PR #687ではexact table scopeと実行上限、PR #689では期間policy、PR #691では型別`WHERE`制約と修正指示、PR #693では完全field path・型・mode・継承制御から成る汎用field policy、PR #695ではphysical table alias・nested identifier・UNNEST検査を実装した。PR #696ではcanonical semanticsから結果roleを導出し、dry runと実行結果の列名・型・modeを共通検査へ接続する。次はplannerとSQL生成からlegacy profile入力、URL、event等の特殊補正を削除する |
