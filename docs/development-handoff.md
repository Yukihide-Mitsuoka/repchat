...
現在は未参照の旧配信実験`tenant_serve.py`を削除し、runtime ratchetと実験記録を更新しています。
...
| UIと補助実行経路 | `tenant_serve.py` | 旧ライブデモ入口とUI補助module、未参照の固定Evidence成果物出力は削除済み。固定対象選択・SQLを持つ未参照の旧配信実験も現在の変更で削除する |
...
| 5 | UI・成果物を中立化 | `visualization_contracts.py`、`visualization_sections.py` | 旧ライブデモ入口とUI payload、未参照の固定Evidence成果物出力は削除済み。`tenant_serve.py`は現在の変更で削除する。残る`event_date`を中立なtemporal roleへ置換し、可視化は契約が対応する意味roleを持つ場合だけ選択する |
