...
現在は、runtimeが生成したcanonical analysis contract artifactをschema／caseごとに受け取り、内容から再計算した
fingerprintを全runへ照合する境界を実装中です。contract内のschema observationも対応するscope snapshotへ照合し、
contract本文はevidenceへ複製しません。
