...
[PR #868](https://github.com/Yukihide-Mitsuoka/repchat/pull/868)はレビュー中です。通常版と診断版の
post-run evidenceを別々に採点し、同一scope・参照記録・計画run・pipeline条件を照合します。
review済みcontractの実際の使用は、この比較だけでは未証明です。
...
| 2 | [#791](https://github.com/Yukihide-Mitsuoka/repchat/issues/791)のbaseline主導SQL精度改善 | [PR #867](https://github.com/Yukihide-Mitsuoka/repchat/pull/867)のpost-run不一致原因reviewはmerge済み。[PR #868](https://github.com/Yukihide-Mitsuoka/repchat/pull/868)の通常・診断evidence offline比較はレビュー中。review済みcontractの生成・利用証明、対抗fixture、baselineは未完了。再現した原因に対応する対象非依存の改善だけを比較する |
