...
現在の受入条件と進行状況は[Issue #622](https://github.com/Yukihide-Mitsuoka/repchat/issues/622)を参照してください。
対象は`run-report.test.ts`に混在するSQL生成担当AIの契約4テストです。既定モデルと未対応temperatureの不使用、structured responseの上限とfinish診断、確定済み分析契約とwarehouse診断を維持するSQL修正、BigQuery compilerのBadRequestだけを修正可能とする判定を`run-report-sql-generation.test.ts`へ移しました。テスト名・fixture・assertion・各テスト内の呼出し順序は変更せず、移動前後の本文一致を確認しました。製品コード、SQL生成prompt・schema・修正契約、timeout、retry、skipも変更しません。移動元は462行から289行になり、専用ファイルは205行です。残るファイルは固定runner除去、SQL guard、BigQuery実行、Evidence出力、未定義語の確認契約に集約されています。
基準はplanner revision／Sankey補正契約を分離した[PR #621](https://github.com/Yukihide-Mitsuoka/repchat/pull/621)とfoundation同期の[PR #613](https://github.com/Yukihide-Mitsuoka/repchat/pull/613)を取り込んだmainです。変更前後の`make test-unit`、変更後の`make test`と`make coverage`は356件成功し、format・lintも成功しました。Nodeのcoverageはline 93.66%、branch 85.26%、function 87.95%で基準と同じです。Python subprocessの行coverageと実ブラウザ目視は対象外です。
