...
現在は時間fieldを持たないschemaの共通契約境界を実装中です。選択schemaに利用可能な時間境界がない場合だけ
...
| 2 | 共通分析契約の自動生成（PR #679〜#684 merge済み、時間なし境界を実装中） | `analysis_contract.py`、`analysis_contract_compiler.py`、`analysis_contract_response.py`、`analysis_contract_generation.py`、`analysis_contract_orchestration.py`、`bigquery_scope_discovery.py` | validator、生成入力、token限定normalizer、構造化response schema、対象非依存prompt、単一Vertex生成I/O、日次shardの検査済みwildcard統合と二段階生成はmerge済み。現在は利用可能な時間境界がない選択schemaだけ`period=null`でcompileし、ingestion-time partitionには標準疑似fieldを合成する。表現不能な必須partition filterは拒否する。完了後は段階3のplanner・SQL・検査をcanonical contractだけへ接続する。手動意味定義、対象別period parser、識別子補正を使わない |
