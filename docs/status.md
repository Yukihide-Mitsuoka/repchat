...
未知例外は伝播させ、2026-09-23にmerge済みである。PR #821で基盤障害を品質分母から除外しつつ
合格不能にする専用run契約を追加した。PR #823でrendererの起動失敗・timeout、PR #825で
preflightの安全な型付き基盤障害を、完全に計測できたrunだけ同契約へ接続し、merge済みである。
...
| **未知の独自nested schemaでのNL→SQL品質** | **評価基盤はfake clientで検証済み、実品質は未検証**。対象別runtimeは削除済み。共通preflightから結果検証・描画、provider usage計測、型付き診断、case別合格gate、失敗分類、予算付き成果物writerまでを実装済み。PR #866で2件の計画runを実runnerとwriterへ通し、PR #867でpost-run原因reviewを追加した。実provider、未知schemaの公式fixture、独立reviewは未検証 | [評価harness](../spikes/schema-generalization-evaluation/README.md)に従い、公式fixtureと独立review、実provider commandを整える。Issue #791では通常・診断evidenceの比較条件を固定し、review済みcontractの利用証明、対抗fixture・baselineへ進む。実行対象と最大費用を提示し個別承認を得た後だけ有料評価する。全gate合格後に製品runtime境界をADRで決める（ADR-0025） |
