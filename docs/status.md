...
まで一本で通っている。根拠は [spikes/report-generation/](../spikes/report-generation/README.md)。

**次の3手（依存順）:**

1. **C7 の決定** — 定義と生成物を**顧客のGitに置くか、こちらに置くか**（ADR-0013 で唯一未決）。
   **これが2の形を決める**（顧客側に置くならビルドパイプラインの構造が変わる）ため先
2. **シェル/データ分離との結線** — 生成したページを、LOG-0025 で実証した**テナント別配信**に載せる。
   現状はローカルビルドで単一テナント
3. **`src/` への移植** — 生成もEvidence統合も、まだスパイクにしか無い

**この順にする理由**: 1 は決定なので安く、2 の作り直しを防ぐ。2 は「生成物が本番アーキテクチャに
収まるか」を測る工程で、ここが通らなければ 3 は早すぎる。
...
| **Evidenceの本番統合** | **生成物が実データで描画されることを確認**（LOG-0073。セッション118,380等、検証済みの値と一致）。認証は `gcloud-cli` で**鍵不要**。**ただしローカルビルド・単一テナント** | シェル/データ分離（LOG-0025）との結線、テナント別データ供給 |
