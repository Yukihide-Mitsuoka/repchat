...
| ③データソース層の隔離（分析データ＝BigQuery） | **テナント別サービスアカウント（ADR-0010 D1）＝未実装** | — |
| ④ペイロード自己検証 | キャッシュ読出時のtenant_id assert | テスト済み |

**正直な現在地**：この多層防御は、**管理データ（Postgres）については①③が本物のバックストップとして成立**しています（アプリ層がWHERE句を落としてもRLSが越境を止める、実証済み）。一方、**分析データ（BigQuery）の③はまだ掛かっていません**。

- LOG-0033 の「実BigQuery 7/7・テナント越境は構造的に不可能」は**②バインダの出力**についての証明であり、正しい。ただしこれは②アプリ層の話で、③データ層の保険ではない。
- 実行時は1つのアイデンティティ（`AdcTokenProvider`）が全テナントのデータセットに到達でき、**データセットはテナント別に選ばれるが資格情報は共通**。ADR-0005 §9.2 が「顧客ごとに1SAなら、ASTバインディングが完全に破綻しても他社データに届かない」と書いた保険が、BigQuery側ではまだ実装されていない（ADR-0010 D1、`datasources.connection_ref` は未使用）。
- したがって「Lookerより安全」は**管理データについては裏付けられ、分析データについては②の正しさに依存している**状態。D1の実装でBigQuery側の③が埋まる。
