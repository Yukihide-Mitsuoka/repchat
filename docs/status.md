...
planning失敗へ閉じる境界を実装し、merge済みである。現在は成功した単一パネルを共通section変換とSQL generatorへ渡し、
