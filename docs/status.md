...
| **未知の独自nested schemaでのNL→SQL品質** | **評価基盤を実装中・品質は未検証**。対象別runtimeは削除済み。共通preflight、実行前計画、失敗attempt記録、参照情報を分離したscorer、runtime入力manifest、manifest駆動preflight、共通planner、単一panel、共通SQL生成、local SQL検証、BigQuery dry run・実行・結果検証・描画への接続と最終run記録は実装済み | 公式fixtureを独立reviewしたうえで、認可済みscopeだけを入力に複数の未知schemaを同一binary・prompt・設定で反復実行して、実値・安全性・費用・描画を照合する（ADR-0025、Issue #188） |
