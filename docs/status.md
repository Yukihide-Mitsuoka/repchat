...
合格不能にする専用run契約を追加した。PR #823でrendererの起動失敗・timeoutを同契約へ接続し、merge済みである。
Issue #824ではpreflightの安全な型付き基盤障害を、完全に計測できた場合だけ同契約へ接続する。
