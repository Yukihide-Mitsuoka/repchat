...
現在は[PR #769](https://github.com/Yukihide-Mitsuoka/repchat/pull/769)で、生成SQLを共通の
SELECT・scope・field・period・partition・可視化出力契約validatorへ渡し、失敗を固定
`sql_validation`／`sql_validation_failed`と安全性booleanへ変換するstageを実装している。
...
| **未知の独自nested schemaでのNL→SQL品質** | **評価基盤を実装中・品質は未検証**。対象別runtimeは削除済み。共通preflight、実行前計画、失敗attempt記録、参照情報を分離したscorer、runtime入力manifest、manifest駆動のpreflight反復境界、共通plannerへの接続、単一plan panelの一意対応、共通SQL生成への接続は実装済み。PR #769で共通local SQL検証への接続を実装中 | 検証済みSQLをdry run・実行以降へ接続し、公式fixtureを独立reviewしたうえで、認可済みscopeだけを入力に複数の未知schemaを同一binary・prompt・設定で反復実行して、実値・安全性・費用・描画を照合する（ADR-0025、Issue #188） |
