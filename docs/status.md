...
**3 の前に、デモを見せられる形にする**（オーナー判断、2026-07-28）。移植は大きく後戻りしにくい一方、
positioning §5 が残した唯一の未検証仮説「BIを入れたが使われていない」は**実装では潰せない**ため。
中身は**起動を1コマンドにすること**と**説明資料**。

**ADR-0014 が決まったことで、3 の中身が変わった。** 生成物の配置先は**顧客のリポジトリが既定**なので、
移植には **Git provider 連携**（コミットAPI・webhook）と**こちらホストのフォールバック**の2経路が要る。
