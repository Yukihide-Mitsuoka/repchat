...
実装し、merge済みである。現在は[PR #763](https://github.com/Yukihide-Mitsuoka/repchat/pull/763)で、
共通分析契約が十分な場合にplannerが不要な初回確認を要求しないよう契約を修正している。
...
| **未知の独自nested schemaでのNL→SQL品質** | **評価基盤を実装中・品質は未検証**。対象別runtimeは削除済み。共通preflight、実行前計画、失敗attempt記録、参照情報を分離したscorer、runtime入力manifestとmanifest駆動のpreflight反復境界は実装済み。現在は十分な共通分析契約から確認なしで初回仕様を生成できるplanner契約を整備中 | 成功したpreflightをplanner以降へ接続し、公式fixtureを独立reviewしたうえで、認可済みscopeだけを入力に複数の未知schemaを同一binary・prompt・設定で反復実行して、実値・安全性・費用・描画を照合する（ADR-0025、Issue #188） |
