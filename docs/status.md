...
### 0.1 次にやること — 「デモを見せられる形にする」（オーナー決定、2026-07-28）
...
| | 中身 | 出来たと言える条件 |
|---|---|---|
| **起動を1コマンドに** | 現状は「ADC → 生成 → Evidenceアプリを別途用意 → sources/pages をコピー → `npm run sources && npm run build` → 配信」の手作業。Evidenceアプリは**リポジトリの外**にあり、セッションが変わると消える | **チェックアウト直後の環境で、1コマンド打てばブラウザでレポートが開く**。Evidenceアプリの用意まで含める。手順の実物は [spikes/report-generation/README.md](../spikes/report-generation/README.md) の「Evidenceで描画する手順（再現）」 |
| **説明資料** | **何が起きているか**（日本語→SQL→検証→ページ）と、**何が担保されているか**（越境不能・未定義は拒否・数値照合）を、実装を知らない人が読める形に | **Looker Studio を使っている人に見せて、5分で違いが伝わる**こと |
...
| Python | スパイクは `spikes/nl2sql-accuracy/.venv` を共用する |
...
- **[.ai/mission.md](../.ai/mission.md) を根拠に判断しない。** 主要ユーザーを「顧客に埋め込みたいベンダー」と
  書いているが、2026-07-27 以降の方針は**直販・ユーザー単位課金**（positioning §6）。
  確定には3点（対象企業の定義・課金区分・認証方式）のオーナー決定が要る**未解消の不整合**
