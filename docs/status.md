...
ただしこの4層はすべて**①テナント分離**（他社データに届かない）に掛かっています。**②同一テナント内の行スコープ**（店舗フィルタ）を守っているのは②のAST書き換えだけで、③④のバックストップは掛かっていません — ADR-0010 D4 / LOG-0040。
...
| **②行スコープのバックストップ**（ADR-0010 D4 / LOG-0040） | **無い**。ASTバインディングが唯一の制御で、`scopeColumn` の書き忘れは静かに全行を返す | ソース非依存のバックストップ選定（成果物ベース／結果セット検証／allowlist必須化）。別ADR |
