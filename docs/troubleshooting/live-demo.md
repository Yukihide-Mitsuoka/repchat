...
## 段階付きSankeyが結果検査で停止する

段階付きSankeyの計画時には同一経路の識別と順序を契約から確認するよう要求しますが、
この選択条件は機械的には未検証です。結果では次を拒否します。

- sourceとtargetに隣接する番号付き段階がない
- 指標値が負数または有限数でない
- 同じsourceとtargetのedgeが重複する
- 行数または段階ごとのnode数が上限を超える

現行の結果検査は経路の完全性やSQLの意味上の来歴を証明しません。停止した場合は分析契約と
表示済みの「BigQueryへ送ったSQL」の対応を確認してください。修正後の有料問い合わせには改めて
費用承認が必要です。
