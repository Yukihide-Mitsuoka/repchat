...
この時点ではBigQueryも実行されていません。

対応chartは削減せず、依頼ごとに順序を中立化した単一のchart enumと、dimensions・measuresの配列へschemaを
...
**Cause:** 生成AIがSQLの`_TABLE_SUFFIX`範囲を問い合わせの対象月と異なる値で返すと、期間検証で停止します。
月前半・後半などの比較では、対象月を読む`_TABLE_SUFFIX`まで部分期間へ狭め、比較条件とpartition filterを
混同する場合があります。従来の修正診断は不一致だけを伝え、期待する完全なsuffix句と条件付き集約の使い分けを
明示していなかったため、SQL担当AIが1回の修正でも解消できないことがありました。期間が一致しないSQLは
BigQueryへ送信してはいけません。

**Fix:** ダッシュボードbuildに限り、期間不一致を実行前診断としてSQL担当AIへ渡し、対象期間・分析内容・出力形状を
変えずに1回だけ修正します。診断には、データソースごとの完全なpartition filterを具体値で明記します。GA4の
月内比較は`_TABLE_SUFFIX`を対象月全体のまま維持し、`event_date`等を使った条件付き集約で現在値・比較値を作ります。
修正後も期間が一致しない場合は、`SQL担当AIで1回修正しましたが、実行前診断を解消できなかったため実行しません`
としてBigQueryへ送信せず停止します。単一グラフ経路は従来どおり即時停止します。

**Prevention:** 月内比較で部分suffixを返した固定SQLに対し、修正AIが
`_TABLE_SUFFIX BETWEEN '20210101' AND '20210131'`と条件付き集約の要件を受け取り、正しい期間のSQLだけが
BigQueryへ渡ることを回帰テストで確認します。修正後も失敗した場合は自動再実行しません。
...
**Fix:** 旧ダッシュボードbuildでは、dry runから返された診断をSQL担当AIへ1回渡し、分析仕様・対象期間・出力形状を
変えずに、選択されたデータソースで利用可能な表現へ修正しました。後に追加されたURL関数名を固定指定する
修正指示は、対象非依存の共通経路に不要なため削除しました。現在の修正依頼は診断文と共通の方言条件だけを渡し、
...
段階付きSankeyは、同一経路ID、段階順序、完全経路を証明する共通契約がないため、planner、相談、
SQL sectionの全入口で停止します。rendererが番号付き隣接edgeを描けることだけでは、入力経路の完全性や
生成SQLの意味上の来歴を証明できません。promptの書き換えや対象別の経路規則では解除しないでください。

一般的な非循環flowで要件を満たす場合は`flow_sankey`を選びます。段階付き経路が必要な場合は、認可scopeから
対象非依存に導出できる同一経路IDと段階順序を共通契約へ追加し、完全経路とSQL来歴のvalidatorを実装してから
再公開します。この停止はVertex AI・BigQuery呼出し前なので、拒否自体にBigQuery費用は発生しません。
