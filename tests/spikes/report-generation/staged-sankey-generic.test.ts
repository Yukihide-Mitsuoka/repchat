...
`], { cwd: ROOT, encoding: 'utf8' });
  ...
  assert.doesNotMatch(output.requirements.join(' '), /ページ|URL|page path|scheme|host|query|fragment/);
});
...
test('staged Sankey renderer preserves arbitrary node values, including URL characters', () => {
  ...
  assert.equal(option.series[0].data[0].label.formatter(), '1. https://example.test/state?phase=beta#part');
  assert.match(option.tooltip.formatter({ dataType: 'edge', data: option.series[0].links[0] }), /https:\/\/example\.test\/state\?phase=beta#part/);
});
